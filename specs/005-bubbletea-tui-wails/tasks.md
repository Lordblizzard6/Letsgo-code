...
- [ ] T070 [P] [US3] TestVitestChatStreaming en `cmd/wails/frontend/src/tests/chat.test.ts`: con bindings mockeados, Chat.svelte acumula los eventos `chat:delta` y renderiza con streaming-markdown + DOMPurify sobre el acumulado (nunca innerHTML) (gui-contract §2, D3, FR-008).
- [ ] T071 [P] [US3] TestVitestComposerSwap en `cmd/wails/frontend/src/tests/composer.test.ts`: el botón alterna Enviar↔Detener según el estado de stream y el input sigue activo durante el stream (gui-contract §2, FR-008).
